...
const ProjectsFrontComponent = ({ data, handleAddButtonClick }) => {
    ...
    const projectTitle = useMemo(() => {
        ...
        if (data.project?.[0].name) {
            return data.project?.[0].name;
        }
        return data.project?.[0].description?.slice(0, 20) ?? '';
    }, [data.project?.[0]]);
    ...
    return (
        <div className={classes.noProject}>
            <Typography variant="h3" component="h3" customClasses={{ container: classes.noProjectTypography }}>
                <FormattedMessage id="Projects2.front.noProject" defaultMessage="You didn't add any projects." />
                {projectTitle}
            </Typography>
            <NoDataButton
                classes={{
...
export const ProjectsFront2 = memo(ProjectsFrontComponent);
